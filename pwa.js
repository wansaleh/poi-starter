...
exports.apply = (api, options = {}) => {
  const pluginOptions = Object.assign(
    {
      ...
      exclude: [/\.git/, /\.map$/, /\.DS_Store/, /^manifest.*\.js(?:on)?$/, /\.gz(ip)?$/, /\.br$/]
    },
  ...
  api.hook('createWebpackChain', config => {
        ...
        [
          {
            ...
            from: normalizePath(
              api.resolveCwd(api.config.publicFolder, '{manifest,manifest}.json')
            ),
            to: '[name].[ext]',
            toType: 'template'
          }
    ...
    config.plugin('pwa-html').use(require('pwa-html-webpack-plugin'), [options, HtmlWebpackPlugin]);
  });
};
